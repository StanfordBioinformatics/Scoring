...
# Yeast genome was too small for standard window size, so we have to output the results of a single partial window instead.  Note the index may be off by one (fix manuall)
	...
	for chr in sorted(chr_sizes):
		...
		for pos in range(1, chr_sizes[chr] + 1):
			...
			if pos % window_size == 0:
				...
				count = 0
		if not chr_sizes[chr] + 1 % window_size == 0:
			window = int((chr_sizes[chr] + 1) / window_size) -1
			if count > 0:
				output.write('\t'.join([chr, str(window), str(count)]) + '\n')
	output.close()
